...
namespace FX {
    ...
    LogicCamera::LogicCamera(const GraphicsWindow& window) : m_window(window) {}
    ...
    LogicObserveCamera::LogicObserveCamera(const GraphicsWindow& window) : LogicCamera(window)
    {
        // 目前画面大小总是和窗口大小一致，因此viewport直接使用window size
        auto size = window.size();
        if (size.x > 0 && size.y > 0)
        {
            m_viewport = { static_cast<float>(window.size().x), static_cast<float>(window.size().y) };
    ...
    void LogicObserveCamera::process()
    {
        ...
        auto& interactor = m_window.interactor();
    ...
    void LogicObserveCamera::observe(const BasicBounding<>& box)
    {
        ...
        // observe函数中的观察方向、观察位置均为定值
        glm::vec3 lookAt = { box.center().x, box.center().y, box.center().z };
        glm::vec3 observeDir = glm::normalize(glm::vec3(-1.0f, -1.0f, -1.0f));
