...
void main()
{
    ...
    vec4 lightDir = normalize(vec4(light, 1) - aPosInView);
    vec3 diffuse = max(dot(lightDir, normal), 0) * aColor.rgb * 0.5;
    vec4 viewDir = vec4(0, 0, 1, 0);
    vec4 halfwayDir = normalize(lightDir + viewDir);
    vec3 specular = pow(max(dot(normal, halfwayDir), 0), 8) * aColor.rgb * 0.1;
