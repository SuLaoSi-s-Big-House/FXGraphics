...
namespace FX {
    ...
    namespace {
        ...
        constexpr inline unsigned int internal(GraphicsTexture::Format format)
        {
            switch (format)
            {
    ...
    void GraphicsTexture::setTransparent(bool isTransparent)
    {
    ...
    bool GraphicsTexture::isTransparent() const
    {
    ...
    void TextureInfo::bind(TextureUnit unit)
    {
        ...
        if (max + 1 > m_textureDepth)
        {
            if (m_textureDepth == 0)
            {
                glTexStorage3D(GL_TEXTURE_2D_ARRAY, 1, internal(pOwner->format()), pOwner->width(), pOwner->height(), max + 1);
            ...
            else
            {
                ...
                glTexStorage3D(GL_TEXTURE_2D_ARRAY, 1, internal(pOwner->format()), pOwner->width(), pOwner->height(), depth);
