...
namespace FX {
    ...
    namespace {
        ...
        inline void windowResizeCallback(GLFWwindow* pWindow, int width, int height)
        {
            assert(pWindow != nullptr);
            auto itr = GraphicsWindowImpl::windowMap().find(pWindow);
        ...
        inline void cursorPositionCallback(GLFWwindow* pWindow, double x, double y)
        {
            assert(pWindow != nullptr);
            auto itr = GraphicsWindowImpl::windowMap().find(pWindow);
            assert(itr->second != nullptr);
            itr->second->interactor().setCursorPos(x, y);
        }

        inline void cursorEnterCallback(GLFWwindow* pWindow, int entered)
        {
            assert(pWindow != nullptr);
            auto itr = GraphicsWindowImpl::windowMap().find(pWindow);
            assert(itr->second != nullptr);
        ...
        inline void mouseButtonCallback(GLFWwindow* pWindow, int point, int action, int mods)
        {
            assert(pWindow != nullptr);
            auto itr = GraphicsWindowImpl::windowMap().find(pWindow);
            assert(itr->second != nullptr);
        ...
        inline void mouseScrollCallback(GLFWwindow* pWindow, double, double yoffset)
        {
            assert(pWindow != nullptr);
            auto itr = GraphicsWindowImpl::windowMap().find(pWindow);
            assert(itr->second != nullptr);
