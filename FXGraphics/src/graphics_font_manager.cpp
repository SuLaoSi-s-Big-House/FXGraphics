...
namespace FX {
    ...
    namespace {
        ...
        int bakeFontBitmap(const stbtt_fontinfo& info, int fontSize, unsigned char* pImage, int width, int height,
            int* pCharacter, int num, std::vector<stbtt_bakedchar>& textData)
        {
    ...
    TextVertex GraphicsFontManager::generate(const Font& font, const std::string& texts)
    {
        TextVertex ret;
        ...
        // 收集所有新的字符（需要生成texture的字符）
        std::vector<int> newTexts;
        {
            std::set<int> textMap;
            for (auto i : codes)
            {
                if (i != '\n' && fontData.textMap.find(i) == fontData.textMap.end())
                {
                    textMap.insert(i);
                }
            }
            newTexts = std::vector<int>(textMap.begin(), textMap.end());
        }

        if (newTexts.empty() == false)
        {
            ...
            for (int i = 0; i < newTexts.size();)
            {
                // n为还未处理的字符的个数
                int n = static_cast<int>(newTexts.size() - i);
        ...
        ret.index.reserve(codes.size() * 7);
        vec2f advance = { 0, 0 };

        for (auto c : codes)
        {
            auto i = fontData.textMap.find(c);
            assert(i != fontData.textMap.end());
            assert(fontData.textArray.size() > i->second.x);
            assert(fontData.textArray[i->second.x].size() > i->second.y);
            assert(fontData.textArray[i->second.x][i->second.y].code == c);

            assert(ret.vertex.size() % 12 == 0);
            unsigned int k = static_cast<unsigned int>(ret.vertex.size()) / 3;

            ret.index.push_back(k); ret.index.push_back(k + 1); ret.index.push_back(k + 2); ret.index.push_back(k + 3);
            ret.index.push_back(RestartMark);

            auto& textData = fontData.textArray[i->second.x][i->second.y].data;

            ret.vertex.push_back(advance.x + textData.xoff);
            ret.vertex.push_back(advance.y + textData.yoff);
            ret.vertex.push_back(0);
            ret.vertex.push_back(advance.x + textData.xoff + textData.x1 - textData.x0);
            ret.vertex.push_back(advance.y + textData.yoff);
            ret.vertex.push_back(0);
            ret.vertex.push_back(advance.x + textData.xoff);
            ret.vertex.push_back(advance.y + textData.yoff + textData.y1 - textData.y0);
            ret.vertex.push_back(0);
            ret.vertex.push_back(advance.x + textData.xoff + textData.x1 - textData.x0);
            ret.vertex.push_back(advance.y + textData.yoff + textData.y1 - textData.y0);
            ret.vertex.push_back(0);
            ...
            ret.uv.push_back(static_cast<float>(i->second.x));
    ...
    GraphicsTexture* GraphicsFontManager::getTexture(const Font& font) const
    {
        auto itr = m_fontMap.find(font);
        if (itr == m_fontMap.end())
        {
            BasicLog::out(BasicLog::kWarn, "The texture has not been generated.");
            return nullptr;
        }
        else
        {
            return itr->second.pTexture.get();
        }
    }
